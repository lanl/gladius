...
typedef struct {
    ...
    // If (1 << n & usageMask) != 0 then this option belongs to option set n.
    uint32_t usageMask;
    ...
    lldb::CommandArgumentType argType;
...
static OptionDefinition gOptions[] = {
    ...
    { LLDB_OPT_SET_3, true, "arch", 'a', required_argument, 0,
      ...
      "running the program.  <architecture> must be one of the architectures "
      "for which the program was compiled."
    },
    { LLDB_OPT_SET_3, true, "file", 'f', required_argument, 0,
      lldb::eArgTypeFilename,
      "Tells the debugger to use the file <filename> "
    ...
    { LLDB_3_TO_5, false, "one-line", 'o', required_argument, 0,
      lldb::eArgTypeNone,
    ...
    { LLDB_3_TO_5, false, "source-before-file", 'S', required_argument, 0,
      lldb::eArgTypeFilename,
      "Tells the debugger to read in and execute the lldb commands in the given "
      "file, before any file provided on the command line has been loaded."
    },
    ...
    { LLDB_3_TO_5, false, "source-quietly", 'Q', no_argument, 0,
        lldb::eArgTypeNone,
...
BuildGetOptTable(
    ...
    for (i = 0, j = 0; i < num_options; ++i) {
        char short_opt = expanded_option_table[i].shortOpt;
        if (option_seen.test(short_opt) == false) {
            getopt_table[j].name    = expanded_option_table[i].longOpt;
            getopt_table[j].has_arg = expanded_option_table[i].option_has_arg;
            getopt_table[j].flag    = NULL;
            getopt_table[j].val     = expanded_option_table[i].shortOpt;
